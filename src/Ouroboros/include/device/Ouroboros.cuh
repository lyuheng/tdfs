...
struct OuroborosChunks : OuroborosBase
{
	...
	static constexpr unsigned int SmallestPageSize_{SMALLEST_SIZE}; //16
	static constexpr unsigned int NumberQueues_{NUMBER_QUEUES}; // 10
	static constexpr unsigned int LargestPageSize_{SmallestPageSize_ << (NumberQueues_ - 1)}; // 2^13=8KB
	static constexpr unsigned int ChunkSize_{ SmallestPageSize_ << (NumberQueues_ - 1) }; // 2^13
	static constexpr unsigned int ChunkAddFactor_{ChunkSize_ / CHUNK_BASE::size_}; // 1
...
struct OuroborosPages : OuroborosBase
{
	...
	static constexpr unsigned int SmallestPageSize_{SMALLEST_SIZE}; //@@: 16 (2^4 or 2^14)
	static constexpr unsigned int NumberQueues_{NUMBER_QUEUES}; //@@: 10
	static constexpr unsigned int LargestPageSize_{SmallestPageSize_ << (NumberQueues_ - 1)}; //2^13=8KB
	static constexpr unsigned int ChunkSize_{ SmallestPageSize_ << (NumberQueues_ - 1) }; // 2^13
	static constexpr unsigned int ChunkAddFactor_{ChunkSize_ / CHUNK_BASE::size_}; // 1
	static constexpr bool s_isBaseOuroboros{ChunkAddFactor_ == 1}; // true
	...
	//align(2^16*4, 2^13) = 2^18
	static constexpr size_t chunk_queue_size_{Ouro::alignment<uint64_t>(chunk_queue_size * sizeof(index_t), ChunkBase::size_)};
	//align(2^23*4, 2^13) = 2^25
	static constexpr size_t page_queue_size_{Ouro::alignment<uint64_t>(QueueType::size_ * sizeof(MemoryIndex), ChunkBase::size_)};

	QueueType d_storage_reuse_queue[NumberQueues_];

	static constexpr size_t memoryManagerSize() { return chunk_queue_size_ + (page_queue_size_ * NumberQueues_); } //2^18 + 2^25*10
	...
	__forceinline__ __device__ bool allocateChunk(index_t& chunk_index)
	{
		...
		if(!s_isBaseOuroboros && QUEUECHUNK) //@@: s_isBaseOuroboros=true, QUEUECHUNK=false
		{
			if(d_base_chunk_reuse_queue->dequeue(chunk_index))
			{
				return true;
		...
		{
			// 如果d_chunk_reuse_queue有chunk，dequeue出来一个chunk_index
			if(d_chunk_reuse_queue.dequeue(chunk_index))
			{
				return true;
			}
