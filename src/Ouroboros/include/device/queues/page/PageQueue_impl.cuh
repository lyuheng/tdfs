...
{
	if (semaphore.signal(1) < size_) //如果#count<2^23,
	{
...
__forceinline__ __device__ void* PageQueue<ChunkType>::allocPage(MemoryManagerType* memory_manager)
{
	...
	semaphore.wait(1, pages_per_chunk, [&]()
	{
		...
		// doesn't allocate memory, it's already allocated
		// only constructs object: set page size and # of pages per chunk
