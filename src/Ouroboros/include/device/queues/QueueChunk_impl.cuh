...
__forceinline__ __device__ void QueueChunk<ChunkBase>::guaranteeWarpSyncPerChunk(index_t position, const char* message, FUNCTION f)
{
	...
	while(true)
	{	
		// 如果condition=false，work_NOT_done就永远是1，这个while（true）就无法退出
...
__forceinline__ __device__ void QueueChunk<ChunkBase>::enqueue(MemoryManagerType* memory_manager, 
...
template <typename ChunkBase>
template <typename MemoryManagerType>
__forceinline__ __device__ void QueueChunk<ChunkBase>::enqueueChunk(MemoryManagerType* memory_manager, 
