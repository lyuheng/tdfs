...
template <unsigned int SMALLEST_PAGE_SIZE, unsigned int SIZE> // SMALLEST_PAGE_SIZE=16, SIZE=2^13
struct QueueIndex
{
	static constexpr int pages_per_chunk_factor{SIZE / SMALLEST_PAGE_SIZE}; // 2^9
	static constexpr int smallest_page_factor{AllocationHelper::static_getNextPow2Pow<unsigned int, SMALLEST_PAGE_SIZE>()}; // 4

	static __device__ __host__ __forceinline__ int getQueueIndex(size_t size) //size是user需要分配多大的内存
	{
		// for example, getNextPow2Pow(32)=5， getNextPow2Pow(33)=6
