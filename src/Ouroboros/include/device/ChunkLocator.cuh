...
struct ChunkLocator
{
	...
	// 第一个参数被除32后就能找到对应的bit所在的int
	// 第二个参数是 chunk_index除32的余数
	// atomicOr just this bit to 1
	...
	// __clz 计算前置0的个数:__clz(0x1F)=__clz(0001 1111)=3
	// __trap() is like exit()
	// 给定一个chunk index，如果他是1就返回这个index，否则往index降低的方向找到第一个不是0的位置index
	...
	// Static Members
	static constexpr int num_bits{Ouro::sizeofInBits<std::remove_pointer<decltype(d_chunk_flags)>::type>()}; // @@:32
	static constexpr int division_factor{countBitShift(num_bits)}; // Divide/Multiply by 32 using shift operator //@@:5
};