...
struct ChunkAccess
{
	...
	static constexpr size_t Chunk_Size_{SIZE}; //2^13
	static constexpr size_t smallest_page_size_{SMALLEST_PAGE}; //16
	// 表示这个chunk的（最多的）所有页需要多少个uint（32bit）
	// (2^13, 16*4*8=2^9)=2^4=16
	...
	// Members
	int count; // Number of available pages //4bytes
	uint32_t size; // Number of pages //4bytes
	MaskDataType availability_mask[MaximumBitMaskSize_]; // Bitmask of how many are free //16*4=64bytes
	...
	{
		return ~(1U << bit); // 111110111111...1111
	}
