...
__forceinline__ __device__ ChunkAccess<SIZE, SMALLEST_PAGE>::FreeMode ChunkAccess<SIZE, SMALLEST_PAGE>::freePage(index_t page_index)
{
	...
	// Set bit to 1
	atomicOr(&availability_mask[mask_index], bit_pattern); // 1=available，0=taken

	// Please do NOT reorder here
	// about __threadfence_block & __threadfence
	// https://www.cnblogs.com/hehe001/archive/2013/01/15/6334729.html
	__threadfence_block();

	auto current_count = atomicAdd(&count, 1) + 1;
	if (current_count == 1)
		return FreeMode::FIRST_FREE; //这个chunk中第一个被free的page
	else if (current_count == size)
		return FreeMode::DEQUEUE; // 所有的page都是free的
...
__forceinline__ __device__ ChunkAccess<SIZE, SMALLEST_PAGE>::Mode ChunkAccess<SIZE, SMALLEST_PAGE>::allocPage(index_t &page_index)
{
	...
	{
		if ((current_count = atomicAdd(&count, 1)) < 0) //这个chunk早已经满了，
			return Mode::CONTINUE;

		// If we observed 0 -> 1, we potentially want to re-enqueue this chunk
		if (current_count == 0) //其他人把count加1了between L46-47，现在count至少是1,这个chunk暂时还是可以使用的
			mode = Mode::RE_ENQUEUE_CHUNK;
	}
	if (current_count == 1) // 刚好可以占用满这个chunk，count已经是0了，
	{
	...
	while (true)
	{
		...
		// __ffsll : Find the position of the first (least significant) bit set to 1 in x,
		...
		while (least_significant_bit = __ffsll(final_mask))
		{
			...
			// current_mask is the ("old") mask
			current_mask = atomicAnd(&availability_mask[bitmask_index % MaximumBitMaskSize_], bit_pattern); //only set this bit to 0

			// Please do NOT reorder here
			__threadfence_block();

			if (checkBitSet(current_mask, least_significant_bit)) //check this bit is 1 before //1=available，0=taken
			{
